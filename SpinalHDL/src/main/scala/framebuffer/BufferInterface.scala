...
case class BufferInterface(config : VGAConfig) extends Bundle with IMasterSlave{
  ...
  val rAddress = UInt(log2Up(config.vDisplayArea) + log2Up(config.hDisplayArea) bits)
  val wValid = Bool
  val wData = Vec(Bits(config.colorR bits), Bits(config.colorG bits), Bits(config.colorB bits))
  val wAddress = UInt(log2Up(config.vDisplayArea) + log2Up(config.hDisplayArea) bits)
